...
class CarlaEnv(Node):
    def __init__(self):
        super().__init__('env')
        print("======== Carla Pubsub Shared Memory Start ========")
        # Subscribers
        ...
        self.timelogger = TimeLogger(warmup_steps = cfg.warmup_steps, total_steps = cfg.total_steps, name="ros2_shm_carla", save_dir=hydra_dir + "/time")
        self.timelogger.mark_start()
        self.fps_log = []
        print("======== Carla Pubsub Shared Memory Ready ========")
    ...
    def send_agent_obs(self, obs):
        msg = BytesMsg()
        msg.data = pickle.dumps(self.seri.serialize(obs))
        self.agent_obs_publisher.publish(msg)
        
    def send_yolo_obs(self, img):
        msg = BytesMsg()
        msg.data = pickle.dumps(self.seri.serialize(img))
